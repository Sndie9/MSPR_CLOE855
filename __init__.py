...
def Readfiche(post_id):
    try:
        ...
        logging.exception("Error reading client fiche")
...
def ReadFicheNom(nom):
    ...
    try:
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM clients WHERE nom = ?', (nom.upper(),))  # Convertir en majuscule pour la recherche
        data = cursor.fetchall()
        conn.close()
        if not data:
            logging.info(f"Client {nom} not found")
            return jsonify({"error": "Client not found"}), 404
        return render_template('read_data.html', data=data)
    except Exception as e:
        logging.exception("Error reading fiche_nom")
        return jsonify({"error": "Internal Server Error"}), 500
...
def ReadBDD():
    try:
        ...
        logging.exception("Error reading BDD")
        return jsonify({"error": "Internal Server Error"}), 500
...
def enregistrer_client():
    try:
        ...
        logging.exception("Error saving client")
        return jsonify({"error": "Internal Server Error"}), 500
